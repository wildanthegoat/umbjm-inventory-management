...
import { ChevronDown, CirclePlus, MoreHorizontal } from "lucide-react";
...
const BarangPage = () => {
  ...
  const { data, isLoading, error, } = useQuery({
      ...
      const filteredData = useMemo(() => {
        if (!data) return [];        
        // First apply the kategori filter
        let filtered = data;
        if (selectedKategori) {
          filtered = data.filter(item => item.kategori.id === selectedKategori);
        }
        
        // Then apply the global search filter
        return filterData(filtered, ["nama_barang", "kondisi", "jumlah", "harga","kategori", "lokasi", "gedung", "ruangan"]);
      ...
      <div className="flex items-center py-4 gap-4">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                {["nama_barang", "kondisi", "jumlah", "harga", "kategori", "lokasi" , "gedung", "ruangan"].map((column) => (
                  <DropdownMenuCheckboxItem
                    key={column}
              ...
              {isLoading
                ...
                : filteredData.map((barang) => (
                    <TableRow key={barang.id}>
                      {visibleColumns.nama_barang && <TableCell>{barang.nama_barang}</TableCell>}
                      {visibleColumns.kondisi && <TableCell>{barang.kondisi}</TableCell>}
                      {visibleColumns.jumlah && <TableCell>{barang.jumlah}</TableCell>}
                      {visibleColumns.harga && <TableCell>{barang.harga}</TableCell>}
                      {visibleColumns.kategori && <TableCell>{barang.kategori.nama_kategori}</TableCell>}
                      {visibleColumns.lokasi && <TableCell>{barang.lokasi.kampus}</TableCell>}
                      {visibleColumns.gedung && <TableCell>{barang.lokasi.gedung}</TableCell>}
                      {visibleColumns.ruangan && <TableCell>{barang.lokasi.ruangan}</TableCell>}
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem>
                            <Link href={`/barang/detail/${barang.id}`}>Detail Barang</Link>
                          </DropdownMenuItem>
